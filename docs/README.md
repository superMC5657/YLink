...
> 一套仿 Xboard 面板风格的代理订阅售卖系统，包含 **用户端应用**（响应式 Web + Tauri 2 桌面端）与 **Go/Gin 服务端**。本文档是全部开发文档的导航与全局约定。
...
| 管理端 | 运营后台（同仓 SPA，18 个管理页面，清单见 [pages.md](frontend/pages.md) §3.14） | Vue 3 SPA（同仓库） | 见 [api/README.md](api/README.md) §16 |
...
| 文档 | 内容 |
|---|---|
| [frontend/README.md](frontend/README.md) | 前端：技术选型、架构、目录结构、工程化 |
...
| [reviews/](reviews/) | 代码评审记录（review-round-01 起，按评审批次冻结的历史快照） |
...
3. **单位与时间约定**：以 [api/README.md §1.4](api/README.md) 为唯一来源（金额传输「元」/库内「分」、流量字节、时间 RFC3339）。
4. **鉴权约定**：免鉴权端点以 [api/README.md §1.2](api/README.md) 为准，其余接口均需 `Authorization: Bearer <access_token>`。
