...
| Web 登录 CORS | ✅ 已修复(2026-08-14):`dev-docker.sh` 构建前端时强制 `VITE_API_BASE_URL=/api/v1`,Web 经 Caddy 同域反代,不再跨端口直连 `localhost:8081`;`configs/config.yaml` CORS 白名单补充 `http://localhost` / `http://127.0.0.1`,直连 8081 的本地 Web 也能通过预检。Tauri 走 plugin-http 原生栈,不受 CORS 影响。已有浏览器若持久化过旧 `app:apiBase`,需在登录页点「重置后端接口地址」 | `scripts/dev-docker.sh`、`server/configs/config.yaml` |
