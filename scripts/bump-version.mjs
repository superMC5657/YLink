 ...
 *   - Cargo.toml / Cargo.lock 只改名为 proxy-seller-web 的 package 段,
 *     不会误伤其它依赖的 version 行。
 */
...
const cargoGet = (s) => s.match(/name = "proxy-seller-web"\nversion = "([^"]+)"/)?.[1]
const cargoSet = (s, v) =>
  s.replace(/name = "proxy-seller-web"\nversion = "[^"]+"/, `name = "proxy-seller-web"\nversion = "${v}"`)
